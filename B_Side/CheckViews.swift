...
struct CheckFocusView: View {
    ...
    init(store: WordStore, onBack: @escaping () -> Void, onClose: @escaping () -> Void) {
        self.store = store; self.onBack = onBack; self.onClose = onClose
        _focusedIDs = State(initialValue: Set(store.words.filter { $0.isFocused }.map { $0.id }))
    }
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(spacing: 0) {
                        ForEach(store.words) { word in
                            // fix 4: show blue for focused words
                            CheckableRow(label: word.term, isChecked: focusedIDs.contains(word.id), accentColor: .blue) {
                                if focusedIDs.contains(word.id) { focusedIDs.remove(word.id) }
                                else { focusedIDs.insert(word.id) }
                            }
                        }
                    }
                }.frame(height: 390)
    ...
    func save() {
        for word in store.words {
            let should = focusedIDs.contains(word.id)
            if word.isFocused != should { store.toggleFocused(id: word.id) }
        }
        onBack()
    }
}
